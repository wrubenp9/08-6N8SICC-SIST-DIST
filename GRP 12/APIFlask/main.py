...
def gerador_senha(tipo_senha):
    ...
    results, columns = consulta_banco(f"SELECT ticket_number FROM Tickets WHERE issued_at BETWEEN "
                                      f"'{date_begin.date()}' AND '{date_end}' AND ticket_type = '{tipo_senha}'"
                                      f" ORDER BY ticket_id DESC")

    if len(results) == 0:
        seq = '01'
    else:
        index = results[0][0].index(tipo_senha)
...
def get_relatorio():
    table = []
    if request.args:
        date_begin = datetime.strptime(str(request.args.get('data_inicio')), "%Y-%m-%d").date() if request.args.\
            get('data_inicio') else '1800-01-01'

        date_end = datetime.strptime(str(request.args.get('data_fim')), "%Y-%m-%d").date() + timedelta(days=1) if \
            request.args.get('data_fim') else '9999-12-31'

        tipo_rel = request.args.get('tipo_rel') if request.args.get('tipo_rel') else '0'
        ...
        table = {'msg_id': 0,
                 'erro': 'É necessário passar parâmetros'}

    return jsonify(table)


# Método que gera uma nova senha com base na quantidade de senhas existentes no banco de dados
@app.route('/gerar_senha_sp', methods=['POST'])
def gerar_senha_sp():
    # tipo_senha = request.args.get('tipo_senha')
    senha = gerador_senha('SP')
    return jsonify(senha)


# Método que gera uma nova senha com base na quantidade de senhas existentes no banco de dados
@app.route('/gerar_senha_se', methods=['POST'])
def gerar_senha_se():
    # tipo_senha = request.args.get('tipo_senha')
    senha = gerador_senha('SE')
    return jsonify(senha)


# Método que gera uma nova senha com base na quantidade de senhas existentes no banco de dados
@app.route('/gerar_senha_sg', methods=['POST'])
def gerar_senha_sg():
    # tipo_senha = request.args.get('tipo_senha')
    senha = gerador_senha('SG')
    return jsonify(senha)
    ...
    if len(results) > 0:
        ...
        if len(results) > 0:
            ...
            if len(results) > 0:
                ...
                dic = {'msg_id': 0,
                       'mensagem': 'Não existe senha na fila'}
                table.append(dic)
